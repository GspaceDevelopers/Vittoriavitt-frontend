...
export default function Loja2() {
  ...
  async function filterpreco() {
    await api.get(`/produto?produto=${produto}`).then((data) => {
  ...
  function ordernarmaisrecente() {
    api.get(`/produto?produto=${produto}`).then((data) => {
  ...
  async function filtercores(item) {
    await api.get(`/categoria?categoria=${produto}`).then((data) => {
  ...
  function cleanfiltrocor() {
    api.get(`/categoria?categoria=${produto}`).then((data) => {
      setDataprodutos(data.data);
    });
  }
